...
const activeColorHex = computed(() => {
  const color = fullPalette.value.find(
    (c) => c.index === activeColorIndex.value
  );
...
function handleWheel(e) {
  ...
  if (e.ctrlKey) {
    ...
    const newZoom = Math.max(
      0.1,
      Math.min(10.0, zoom.value + direction * zoomIntensity * zoom.value)
    );
...
function handleTouchStart(e) {
  if (e.touches.length === 1) {
    ...
    dragStart.value = {
      x: t1.clientX - panX.value,
      y: t1.clientY - panY.value,
    };
  }
}
...
async function handleClick(e) {
  ...
  if (currentTool.value === 'brush') {
    // Voltamos ao await simples para estabilidade total
...
async function handleColorPicked(e) {
  try {
    ...
    showToast("Limite atingido", "warning");
  }
}
...
<template>
  <div class="canvas-wrapper">
      ...
      <div class="hud-inner custom-scroll">
        ...
        <div class="section">
          ...
          <div class="grid-tools">
            ...
            <div style="display: flex; gap: 4px">
              <button
                @click="undoLastAction().then(refresh)"
                title="Desfazer"
                style="flex: 1"
              >
                <Undo2 :size="18" />
              </button>
              <button @click="redo" title="Refazer" style="flex: 1">
                <Redo2 :size="18" />
              </button>
            </div>
          </div>
        ...
        <div class="section">
          <button
            @click="toggleRuler"
            :class="{
              active: currentTool === 'ruler' || highlightedRow !== -1,
            }"
            class="wide-btn"
          >
          ...
          <div v-if="highlightedRow !== -1" class="ruler-panel">
            <div class="nav-row">
              <button @click="moveRow(1)" class="nav-btn">
                <ChevronUp :size="20" />
              </button>
              <div class="row-num">
            ...
            <div v-if="rowSummary.length" class="summary-box">
              <div
                class="dir-header"
                :class="{
                  'dir-left': highlightedRow % 2 !== 0,
                  'dir-right': highlightedRow % 2 === 0,
                }"
              >
                <ArrowLeft
                  v-if="highlightedRow % 2 !== 0"
                  :size="20"
                  stroke-width="2.5"
                />
          ...
          <div v-if="selectionRect" class="selection-status">
            ...
            <button @click="selectionRect = null" class="btn-close-sel">
              <X :size="14" />
            </button>
          </div>
        </div>
    ...
    >
      ...
      >
        ...
        <div
          ...
          :style="{
            left: BACKEND_MARGIN + selectionRect.x * CELL_SIZE + 'px',
            top: BACKEND_MARGIN + selectionRect.y * CELL_SIZE + 'px',
            width: selectionRect.w * CELL_SIZE + 'px',
            height: selectionRect.h * CELL_SIZE + 'px',
          }"
        ></div>
      </div>
    </div>
    <div class="shortcuts-guide">
      ...
      <div class="shortcut-item">
        <span class="key">Ctrl</span> + <span class="key">Scroll</span>
        <span>Zoom</span>
      </div>
      <div class="shortcut-item">
        <span class="key">Espaço</span> + <span class="key">Clique</span>
        <span>Arrastar</span>
      </div>
    </div>
  </div>
